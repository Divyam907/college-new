...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user[3], password):
            if user[4] == 'teacher':
                flash('Please use the Teacher login portal.', 'warning')
                return redirect(url_for('teacher_login'))
            session.update(user_id=user[0], name=user[1], email=user[2], role=user[4])
            return redirect(url_for('dashboard'))
...
def teacher_login():
    """Teacher login page (separate portal)."""
    if request.method == 'POST':
        email    = request.form.get('email', '').strip()
        password = request.form.get('password', '')
        conn = get_conn(); cur = conn.cursor()
        ...
        if user and check_password_hash(user[3], password):
            if user[4] != 'teacher':
                flash('Please use the College/Admin login portal.', 'warning')
                return redirect(url_for('login'))
            session.update(user_id=user[0], name=user[1], email=user[2], role=user[4])
...
def teacher_signup():
    ...
    if request.method == 'POST':
        ...
        if not all([name, email, password]):
            ...
            return render_template('teacher/login.html', show_signup=True)
        try:
            conn = get_conn(); cur = conn.cursor()
            cur.execute(
                'INSERT INTO users (name,email,password_hash,role) VALUES (%s,%s,%s,%s)',
...
def api_teacher_branches():
    ...
    if batch_id:
        cur.execute("""
            SELECT DISTINCT b.branch_id, b.name, b.code
            FROM branch b
            JOIN class c ON c.branch_id=b.branch_id
            WHERE c.batch_id=%s
            ORDER BY b.name
        """, (int(batch_id),))
    else:
...
@teacher_required
def api_teacher_sections(class_id):
    conn = get_conn(); cur = conn.cursor()
    cur.execute('SELECT section_id, name FROM section WHERE class_id=%s ORDER BY name', (class_id,))
    rows = cur.fetchall(); cur.close(); conn.close()
    return jsonify([{'id': r[0], 'name': r[1]} for r in rows])
...
@teacher_required
def api_teacher_periods(section_id):
...
def teacher_send_report():
    ...
    send_to_authorities = data.get('send_to_authorities') in ('true', 'on', True, '1')
    send_to_parents = data.get('send_to_parents') in ('true', 'on', True, '1')
    ...
    try:
        ...
        # Build attendance query based on selection
        att_query = """
        ...
        if section_id:
            att_query += " AND a.section_id=%s"; params.append(int(section_id))
        elif class_id:
            att_query += " AND st.class_id=%s"; params.append(int(class_id))
        elif branch_id:
            att_query += " AND c.branch_id=%s"; params.append(int(branch_id))
        elif batch_id:
            att_query += " AND c.batch_id=%s"; params.append(int(batch_id))

        att_query += " ORDER BY a.date, c.name, sec.name, st.roll_no, t.from_time"
        cur.execute(att_query, params)
        records = cur.fetchall()

        # Also get all students for that filter to mark absent
        stu_query = """
            SELECT st.std_id, st.name, st.roll_no, c.name as class_name,
                   sec.name as section_name, br.name as branch_name
            FROM student st
            LEFT JOIN class c ON st.class_id=c.class_id
        ...
        # Get periods for this section/class
        period_query = """
        ...
        p_params = []
        if section_id:
            period_query += " AND t.section_id=%s"; p_params.append(int(section_id))
        elif class_id:
            period_query += " AND sec.class_id=%s"; p_params.append(int(class_id))
        period_query += " ORDER BY t.from_time"
        cur.execute(period_query, p_params)
        periods = cur.fetchall()

        # Build recipients list
        recipients = []
        if send_to_authorities:
            # Get appropriate authorities based on hierarchy
            if batch_id and not branch_id:
                # Batch level → Director/higher authorities
                cur.execute("SELECT email FROM authority WHERE role IN ('director','dean')")
                recipients.extend([r[0] for r in cur.fetchall()])
            if branch_id:
                # Branch level → HOD
                cur.execute("SELECT hod_email FROM branch WHERE branch_id=%s AND hod_email IS NOT NULL",
                           (int(branch_id),))
                row = cur.fetchone()
                if row and row[0]:
                    recipients.append(row[0])
                cur.execute("SELECT email FROM authority WHERE branch_id=%s", (int(branch_id),))
                recipients.extend([r[0] for r in cur.fetchall()])

        if send_to_parents:
            # Get parent emails for students in selection
            parent_query = "SELECT DISTINCT pc.parent_email FROM parent_contact pc JOIN student st ON pc.student_id=st.std_id WHERE 1=1"
            pp = []
            if section_id:
                parent_query += " AND st.section_id=%s"; pp.append(int(section_id))
            elif class_id:
                parent_query += " AND st.class_id=%s"; pp.append(int(class_id))
            elif branch_id:
                parent_query += " AND st.class_id IN (SELECT class_id FROM class WHERE branch_id=%s)"; pp.append(int(branch_id))
            cur.execute(parent_query, pp)
            recipients.extend([r[0] for r in cur.fetchall()])

        cur.close(); conn.close()

        # Deduplicate recipients
        recipients = list(dict.fromkeys([e for e in recipients if e]))

        if not recipients:
            return jsonify({'error': 'No recipients found. Please configure authorities/parent contacts.'}), 400
        ...
        try:
            ...
            # Header
            ws.append([f"Attendance Report: {from_date} to {to_date}"])
            ws.append([])

            # Build per-day, per-period attendance matrix
            headers = ['Roll No', 'Student Name', 'Class', 'Section', 'Branch']
            ...
            for date in date_range:
                for p in periods:
                    headers.append(f"{date.strftime('%d/%m')} {p[0]}")
                if not periods:
            ...
            for stu in all_students:
                row = [stu[2] or '', stu[1], stu[3] or '', stu[4] or '', stu[5] or '']
                for date in date_range:
                    for p in periods:
                        ...
                        if key in present_set:
                            ...
                            row.append('A')
                    if not periods:
                        # Check any attendance that day
                        any_present = any((str(date), stu[1], '') in present_set for _ in [1])
                        row.append('P' if any_present else 'A')
                ws.append(row)
            ...
            # Fallback: use pandas if openpyxl not available
            if pd is not None:
                report_data = []
                for stu in all_students:
                    row_data = {'Roll No': stu[2], 'Name': stu[1], 'Class': stu[3], 'Section': stu[4]}
                    for date in date_range:
                        for p in periods:
                            key = (str(date), stu[1], p[0])
                            col_name = f"{date.strftime('%d/%m')} {p[0]}"
                            row_data[col_name] = 'P' if key in present_set else 'A'
        ...
        body = f"Please find attached the attendance report from {from_date} to {to_date}."
        errors = []
        ...
        if errors:
            return jsonify({'success': True, 'sent_to': len(recipients) - len(errors),
                           'errors': errors}), 207
        return jsonify({'success': True, 'sent_to': len(recipients), 'recipients': recipients})
...
def api_college_delete_batch(batch_id):
    ...
    try:
        ...
        cur.execute('DELETE FROM timetable WHERE section_id IN (SELECT section_id FROM section WHERE class_id IN (SELECT class_id FROM class WHERE batch_id=%s))', (batch_id,))
        cur.execute('DELETE FROM engagement_log WHERE period_id IN (SELECT tt_id FROM timetable WHERE section_id IN (SELECT section_id FROM section WHERE class_id IN (SELECT class_id FROM class WHERE batch_id=%s)))', (batch_id,))
        cur.execute('DELETE FROM student WHERE class_id IN (SELECT class_id FROM class WHERE batch_id=%s)', (batch_id,))
        cur.execute('DELETE FROM section WHERE class_id IN (SELECT class_id FROM class WHERE batch_id=%s)', (batch_id,))
...
def api_college_branches():
    ...
    cur.execute('SELECT branch_id, name, code, hod_name, hod_email FROM branch ORDER BY name')
    rows = cur.fetchall(); cur.close(); conn.close()
    return jsonify([{'id': r[0], 'name': r[1], 'code': r[2], 'hod_name': r[3], 'hod_email': r[4]} for r in rows])
...
def api_college_add_branch():
    ...
    try:
        conn = get_conn(); cur = conn.cursor()
        cur.execute('''INSERT INTO branch (name, code, hod_name, hod_email) VALUES (%s, %s, %s, %s)
                       RETURNING branch_id''',
                    (name, code or None, hod_name or None, hod_email or None))
...
def api_college_delete_branch(branch_id):
    conn = get_conn(); cur = conn.cursor()
    try:
        cur.execute('DELETE FROM timetable WHERE section_id IN (SELECT section_id FROM section WHERE class_id IN (SELECT class_id FROM class WHERE branch_id=%s))', (branch_id,))
        cur.execute('DELETE FROM student WHERE class_id IN (SELECT class_id FROM class WHERE branch_id=%s)', (branch_id,))
        cur.execute('DELETE FROM section WHERE class_id IN (SELECT class_id FROM class WHERE branch_id=%s)', (branch_id,))
...
def api_college_delete_class(class_id):
    conn = get_conn(); cur = conn.cursor()
    try:
        cur.execute('DELETE FROM engagement_log WHERE section_id IN (SELECT section_id FROM section WHERE class_id=%s)', (class_id,))
        cur.execute('DELETE FROM attendance WHERE section_id IN (SELECT section_id FROM section WHERE class_id=%s)', (class_id,))
        cur.execute('DELETE FROM timetable WHERE section_id IN (SELECT section_id FROM section WHERE class_id=%s)', (class_id,))
...
def api_college_generate_timetable(section_id):
    ...
    try:
        ...
        recess_minutes = recess_duration
...
def api_college_teachers():
    conn = get_conn(); cur = conn.cursor()
    cur.execute("SELECT id, name, email FROM users WHERE role='teacher' ORDER BY name")
    rows = cur.fetchall(); cur.close(); conn.close()
...
def api_college_students():
    ...
    query = '''SELECT st.std_id, st.name, st.email, st.roll_no, c.name as class_name,
                      s.name as sec_name, st.dob, st.class_id, st.section_id
               FROM student st
               LEFT JOIN class c ON st.class_id=c.class_id
...
def api_college_update_student(std_id):
    ...
    updates = []
    params = []
    for field in ['name', 'email', 'roll_no', 'dob']:
        if field in data:
